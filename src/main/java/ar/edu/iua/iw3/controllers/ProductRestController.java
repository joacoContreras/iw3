...
public class ProductRestController extends BaseRestController {
    ...
    public ResponseEntity<?> add(@RequestBody Product product) {
        try {
            Product res = productBusiness.add(product);
            HttpHeaders responseHeaders = new HttpHeaders();
            responseHeaders.set("location", Constants.URL_PRODUCTS + "/" + res.getId());
    ...
    public ResponseEntity<?> addCategory(@RequestBody Category category) {
        try {
            Category res = categoryBusiness.add(category);
            HttpHeaders responseHeaders = new HttpHeaders();
            responseHeaders.set("location", Constants.URL_PRODUCTS + "/categories/" + res.getId());
            return new ResponseEntity<>(responseHeaders, HttpStatus.CREATED);
        } catch (BusinessException e) {
            return new ResponseEntity<>(response.build(HttpStatus.INTERNAL_SERVER_ERROR, e, e.getMessage()),
                    HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    @PutMapping(value = "/categories", consumes = MediaType.APPLICATION_JSON_VALUE)
